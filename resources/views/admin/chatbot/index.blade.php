                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition" x-data="{ editing: false }">
                        <td class="px-4 py-3 sm:px-6 sm:py-4 align-top">
                            ...
                            <input x-show="editing" form="edit-form-{{ $knowledge->id }}" type="text" name="topic" value="{{ $knowledge->topic }}" class="w-full text-xs border border-gray-300 rounded mb-1 p-1.5 focus:ring-blue-500">
                            <input x-show="editing" form="edit-form-{{ $knowledge->id }}" type="text" name="intent_name" value="{{ $knowledge->intent_name }}" class="w-full text-xs border border-gray-300 rounded p-1.5 focus:ring-blue-500">
                        </td>
                        <td class="px-4 py-3 sm:px-6 sm:py-4 align-top text-gray-500 text-xs sm:text-sm">
                            ...
                            <textarea x-show="editing" form="edit-form-{{ $knowledge->id }}" name="keywords" class="w-full text-xs border border-gray-300 rounded p-1.5 focus:ring-blue-500" rows="3">{{ implode(', ', $kwArr) }}</textarea>
                        </td>
                        <td class="px-4 py-3 sm:px-6 sm:py-4 align-top text-gray-500 text-xs sm:text-sm">
                            <div x-show="!editing" class="line-clamp-3">{{ $knowledge->response }}</div>
                            <textarea x-show="editing" form="edit-form-{{ $knowledge->id }}" name="response" class="w-full text-xs border border-gray-300 rounded p-1.5 focus:ring-blue-500" rows="4">{{ $knowledge->response }}</textarea>
                        </td>
