...
@section('title', isset($app) ? 'Edit Aplikasi' : 'Tambah Aplikasi')
@section('header_title', isset($app) ? 'Edit Aplikasi' : 'Tambah Aplikasi Baru')

@section('content')
<div class="max-w-5xl mx-auto" x-data="saasForm({{ isset($app) ? json_encode($app) : '{}' }})">
    
    <form action="{{ isset($app) ? route('admin.saas.update', $app->id) : route('admin.saas.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" x-model="form.name" class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Slug (URL)</label>
                    <input type="text" name="slug" x-model="form.slug" class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="price" x-model="form.price" class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tagline Singkat</label>
                    <input type="text" name="tagline" x-model="form.tagline" class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Lengkap & Fitur</label>
                    <textarea name="description" rows="5" x-model="form.description" class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"></textarea>
                ...
                <template x-for="(plan, key) in plans" :key="key">
                    <div class="p-4 border border-gray-200 rounded-lg bg-gray-50">
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-2">
                            <input type="text" :name="`plans[${key}][name]`" x-model="plan.name" placeholder="Nama Paket (ex: Starter)" class="text-sm border-gray-300 rounded">
                            <input type="number" :name="`plans[${key}][price]`" x-model="plan.price" placeholder="Harga" class="text-sm border-gray-300 rounded">
                        </div>
                        {{-- Fitur Paket --}}
                        <textarea :name="`plans[${key}][features_raw]`" x-model="plan.features_raw" rows="2" class="w-full text-sm border-gray-300 rounded" placeholder="Fitur dipisah koma (ex: 5 User, 10GB)"></textarea>
                    </div>
                </template>
            </div>
        </div>
